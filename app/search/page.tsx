...
import { Search, ChevronRight, HelpCircle, Tag } from "lucide-react";
import Link from "next/link";
import { searchArticles, getArticlesByTag } from "@/services/articles";
...
export async function generateMetadata({ searchParams }: SearchPageProps) {
  ...
  const label = tag || category || q;
  return {
    title: label ? `Search results for "${label}" | ${siteConfig.name}` : `Search | ${siteConfig.name}`,
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const activeQuery = tagQuery || categoryQuery || searchQuery;
  const isTagSearch = !!tagQuery;
  const isCategorySearch = !!categoryQuery;

  let searchResults;
  if (tagQuery) {
    searchResults = await getArticlesByTag(tagQuery, 24);
    // If tag search has no direct tag column match, fallback to broader query matching (title, excerpt, etc.)
    if (searchResults.length === 0) {
      searchResults = await searchArticles(tagQuery, 24);
    }
  } else if (categoryQuery) {
    // Normalise category slug back to db format (e.g. university-admissions -> university admissions)
    const normalizedCategory = categoryQuery.replace(/-/g, " ");
    searchResults = await searchArticles(normalizedCategory, 24);
  } else {
    searchResults = await searchArticles(searchQuery, 24);
  }
    ...
    <div className="space-y-10">
      ...
      <header className="space-y-3 border-b border-border pb-6">
        <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-primary bg-primary/10 px-3 py-1 rounded-md">
          {isTagSearch ? <Tag className="h-3.5 w-3.5" /> : <Search className="h-3.5 w-3.5" />}
          {isTagSearch ? "Tag Search" : isCategorySearch ? "Category Search" : "Search Portal"}
        </span>
        <h1 className="font-heading text-3xl font-extrabold text-foreground tracking-tight">
          {activeQuery ? `Results for \"${activeQuery}\"` : "Search Platform"}
        </h1>
        ...
        activeQuery && (
          <div className="text-center py-16 bg-secondary/15 rounded-3xl border border-dashed border-border max-w-md mx-auto flex flex-col items-center">
            ...
            <p className="text-xs text-muted-foreground px-6 leading-relaxed">
              We couldn&apos;t find any articles matching &quot;{activeQuery}&quot;. Try searching for categories like &quot;Admissions&quot;, &quot;Scholarships&quot;, &quot;Internships&quot;, or &quot;Future Skills&quot;.
            </p>
          </div>
        )
      )}
