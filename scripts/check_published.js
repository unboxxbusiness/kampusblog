...
const logPath = path.join(__dirname, '..', 'check_out.txt');
  ...
  "requests": [
    {
      "type": "execute",
      "stmt": {
        "sql": "SELECT slug, title FROM articles ORDER BY published_at DESC LIMIT 5"
      }
    },
    {
      "type": "close"
...
const req = https.request(pipelineUrl, options, (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      let outText = 'Recent Articles:\n';
      if (rows && rows.length > 0) {
        rows.forEach(r => {
          outText += `slug: ${r[0].value} | title: ${r[1].value}\n`;
        });
      } else {
        outText += 'No articles found';
      }
      fs.writeFileSync(logPath, outText, 'utf8');
    } catch (err) {
