...
out_lines = ["=== LIVE CHECK ==="]
for idx, slug in enumerate(SLUGS, 1):
    url = f"https://kampusfilter.com/articles/{slug}"
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
        with urllib.request.urlopen(req, context=ctx) as resp:
            out_lines.append(f"{idx}. [200 OK] {url}")
    except urllib.error.HTTPError as e:
        out_lines.append(f"{idx}. [{e.code} ERROR] {url}")
    except Exception as e:
        out_lines.append(f"{idx}. [ERR: {e}] {url}")

with open(r"e:\brandapp\kampusfilter\verify_live_urls_output.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out_lines))
