...
rawContent = """<p>UNESCO has officially partnered with Coursera and LG AI Research to roll out the Global MOOC on the Ethics of AI. This free initiative aims to equip students, researchers, and professionals with fundamental skills in responsible technology deployment.</p>
...
res = []
res.append(f"Original length: {len(rawContent)}")
res.append(f"Original contains geo-mermaid? {'geo-mermaid' in rawContent}")
res.append(f"Original contains geo-dates-table? {'geo-dates-table' in rawContent}")

# Run old regex stripping logic from geo-parser.ts
stripped_old = re.sub(r'<div[^>]*class=["\']geo-takeaways["\'][^>]*>[\s\S]*?</div>', '', rawContent, flags=re.I)
stripped_old = re.sub(r'<div[^>]*class=["\']geo-citations["\'][^>]*>[\s\S]*?</div>', '', stripped_old, flags=re.I)
stripped_old = re.sub(r'<div[^>]*class=["\']geo-faq["\'][^>]*>[\s\S]*?</div>\s*</div>', '', stripped_old, flags=re.I)

res.append(f"Old stripped length: {len(stripped_old)}")
res.append(f"Old stripped contains geo-mermaid? {'geo-mermaid' in stripped_old}")
res.append(f"Old stripped contains geo-dates-table? {'geo-dates-table' in stripped_old}")

# Run new regex stripping logic from geo-parser.ts
stripped_new = re.sub(r'<div[^>]*class=["\']geo-takeaways["\'][^>]*>[\s\S]*?</div>', '', rawContent, flags=re.I)
stripped_new = re.sub(r'<div[^>]*class=["\']geo-citations["\'][^>]*>[\s\S]*?</div>', '', stripped_new, flags=re.I)
stripped_new = re.sub(r'<div[^>]*class=["\']geo-faq["\'][^>]*>[\s\S]*', '', stripped_new, flags=re.I)

res.append(f"New stripped length: {len(stripped_new)}")
res.append(f"New stripped contains geo-mermaid? {'geo-mermaid' in stripped_new}")
res.append(f"New stripped contains geo-dates-table? {'geo-dates-table' in stripped_new}")

with open("e:/brandapp/kampusfilter/test_parser_out.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(res))
