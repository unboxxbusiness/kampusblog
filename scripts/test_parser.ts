...
const rawContent = `<p>UNESCO has officially partnered with Coursera and LG AI Research to roll out the Global MOOC on the Ethics of AI. This free initiative aims to equip students, researchers, and professionals with fundamental skills in responsible technology deployment.</p>
...
const parsed = parseGEOContent(rawContent);

let out = [];
out.push('=== GEO PARSER RESULT ===');
out.push('Takeaways extracted count: ' + parsed.takeaways.length);
out.push('Citations extracted count: ' + parsed.citations.length);
out.push('FAQs extracted count: ' + parsed.faqs.length);
out.push('cleanContent contains geo-mermaid? ' + parsed.cleanContent.includes('geo-mermaid'));
out.push('cleanContent contains geo-dates-table? ' + parsed.cleanContent.includes('geo-dates-table'));
out.push('cleanContent snippet around table: ' + parsed.cleanContent.substring(parsed.cleanContent.indexOf('geo-dates-table') - 50, parsed.cleanContent.indexOf('geo-dates-table') + 200));

fs.writeFileSync(path.join(__dirname, '..', 'public', 'test_res.txt'), out.join('\n'));
