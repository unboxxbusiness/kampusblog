...
export default function FeaturedArticleCard({ article }: FeaturedArticleCardProps) {
  ...
  return (
    <article className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 grid grid-cols-1 lg:grid-cols-12">
      ...
      <div className="relative lg:col-span-7 aspect-[16/10] lg:aspect-auto min-h-[300px] lg:min-h-[420px] overflow-hidden flex items-center justify-center lg:border-r border-border">
        <DynamicEditorialBanner title={title} category={category} className="w-full h-full" />
        
        {/* Category Overlay */}
        <span className="absolute top-6 left-6 bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-widest px-3.5 py-1.5 rounded-lg shadow-md border border-primary/20">
          Featured: {category}
        </span>
      </div>
      ...
      <div className="p-6 sm:p-8 lg:col-span-5 flex flex-col justify-center">
        ...
        <h2 className="font-heading text-2xl sm:text-3xl font-extrabold text-foreground group-hover:text-primary transition-colors leading-tight mb-4">
        ...
        {/* Author information & Read Button */}
        <div className="mt-auto pt-6 border-t border-border flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-secondary flex items-center justify-center font-bold text-sm text-primary uppercase border border-border">
              {author.slice(0, 2)}
            </div>
            <div>
          ...
          <Link
            href={`/articles/${slug}`}
            className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground font-semibold px-5 py-2.5 rounded-xl hover:bg-primary/95 transition-colors shadow-md text-sm group-hover:shadow-lg"
          >
            Read Article
